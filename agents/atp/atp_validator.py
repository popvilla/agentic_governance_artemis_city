...
from .atp_models import ATPMessage, ATPMode, ATPActionType
...
class ATPValidator:
    ...
    def validate(self, message: ATPMessage) -> ValidationResult:
        ...
        if message.has_atp_headers and not message.is_complete:
            ...
            if self.strict:
                result.add_error(
                    f"Incomplete ATP headers: missing {', '.join(missing)}"
                )
            else:
                result.add_warning(
                    f"Recommended ATP headers missing: {', '.join(missing)}"
                )
        ...
        if not message.content or len(message.content.strip()) == 0:
            ...
            result.add_warning(
                f"Message content is very short ({len(message.content)} chars)"
            )
        elif len(message.content) > self.MAX_CONTENT_LENGTH:
            result.add_suggestion(
                "Consider breaking long message into multiple ATP messages"
            )
