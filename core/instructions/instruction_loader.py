...
class InstructionLoader:
    ...
    PROJECT_FILES = ["codex.md", "WARP.md", ".codex/instructions.md", ".artemis/instructions.md"]

    LOCAL_FILES = ["codex.md", "instructions.md", ".instructions.md"]
    ...
    def _load_global_instructions(self) -> Optional[InstructionScope]:
        ...
        for file_pattern in self.GLOBAL_FILES:
            ...
            if content:
                return InstructionScope(level="global", path=file_path, content=content, priority=1)
        return None
        ...
        for filename in self.PROJECT_FILES:
            ...
            if content:
                return InstructionScope(
                    level="project", path=file_path, content=content, priority=2
                )
        return None
    ...
    def _load_local_instructions(self, current_dir: str) -> Optional[InstructionScope]:
        ...
        for filename in self.LOCAL_FILES:
            ...
            content = self._read_file(file_path)
            if content:
                return InstructionScope(level="local", path=file_path, content=content, priority=3)
        return None
    ...
    def _load_agent_instructions(self, agent_name: str) -> Optional[InstructionScope]:
        ...
        for filename in ["instructions.md", f"{agent_name}.md"]:
            ...
            if content:
                return InstructionScope(
                    level=f"agent:{agent_name}", path=file_path, content=content, priority=4
                )
        return None
