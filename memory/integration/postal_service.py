...
from .memory_client import MemoryClient, MCPResponse
from .trust_interface import get_trust_interface, TrustLevel
...
class PostOffice:
    ...
    def send_mail(
        ...
        if response.success:
            ...
            self.memory_client.manage_tags(
                vault_path,
                'add',
                [sender, recipient, 'postal-mail', priority]
            )
        else:
    ...
    def check_mailbox(
        ...
        mail = self.context_loader.search_context(
            f"#{agent_name} #postal-mail",
            limit=20
        )
    ...
    def send_to_archives(
        ...
        if not self.trust_office.can_perform_operation(sender, 'write'):
            ...
            return MCPResponse(
                success=False,
                error="Insufficient clearance for archival operations"
            )
        ...
        response = self.memory_client.store_agent_context(
            sender,
            content,
            folder=f"Archives/{archive_section}"
        )
