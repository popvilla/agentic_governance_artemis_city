...
if check_file ".env"; then
    # Generate MCP API key first
    MCP_KEY=$(generate_key)

    if [ -f ".env.example" ]; then
        cp .env.example .env
        echo -e "${GREEN} Copied from .env.example${NC}"
    else
        echo -e "${YELLOW}  .env.example not found, creating default template${NC}"
        cat > .env << EOF
...
# Obsidian Integration (get from Obsidian Settings → Local REST API)
