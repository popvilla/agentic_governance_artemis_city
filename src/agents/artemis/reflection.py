...
class ConceptGraph:
    ...
    def get_top_concepts(self, n: int = 10) -> List[ConceptNode]:
        ...
        sorted_concepts = sorted(
            self.concepts.values(), key=lambda c: (c.importance_score, c.frequency), reverse=True
        )
...
class ReflectionEngine:
    ...
    def synthesize(self, focus: Optional[str] = None) -> str:
        ...
        if top_concepts:
            ...
            for i, concept in enumerate(top_concepts[:5], 1):
                parts.append(f"{i}. **{concept.concept}** " f"(appeared {concept.frequency} times)")
            parts.append("")
        ...
        if clusters:
            ...
            for i, cluster in enumerate(clusters[:3], 1):
                concept_names = [self.concept_graph.concepts[c].concept for c in cluster]
    ...
    def find_connections(self, concept: str) -> List[str]:
        ...
        if concept_key in self.concept_graph.concepts:
            ...
            return [self.concept_graph.concepts[c].concept for c in node.related_concepts]
        return []
    ...
    def _identify_relationships(self, concepts: List[str], context: str) -> None:
    ...
    def _build_narrative(self, top_concepts: List[ConceptNode], clusters: List[Set[str]]) -> str:
        ...
        if len(top_concepts) > 1:
            ...
            narrative_parts.append(
                f"This connects to {', '.join(related)}, " f"forming a cohesive system of ideas."
            )
